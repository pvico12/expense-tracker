...
@router.get("/subscription", response_model=GoalSubscriptionResponse)
def get__single_user_goal_subscription(goal_sub_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """
    Retrieve a specific goal subscription for user.
    """
    subscription = db.query(GoalSubscription).filter(GoalSubscription.user_id == current_user.id, GoalSubscription.id == goal_sub_id).first()
    if not subscription:
        raise HTTPException(status_code=404, detail="Subscription not found")
    return subscription

@router.get("/subscriptions", response_model=List[GoalSubscriptionResponse])
def get_user_goal_subscriptions(
        current_user: User = Depends(get_current_user),
        db: Session = Depends(get_db)
    ):
    subscriptions = db.query(GoalSubscription).filter(GoalSubscription.user_id == current_user.id).all()
    return subscriptions

@router.post("/subscription", response_model=GoalSubscriptionResponse, status_code=status.HTTP_201_CREATED)
def create_goal_subscription(subscription: GoalSubscriptionLocation, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    new_subscription = GoalSubscription(
        user_id=current_user.id,
...
@router.put("/subscription/{goal_sub_id}", response_model=GoalSubscriptionResponse)
def update_goal_subscription(
        goal_sub_id: int,
        subscription: GoalSubscriptionLocation,
        current_user: User = Depends(get_current_user),
        db: Session = Depends(get_db)
    ):
    subscription = db.query(GoalSubscription).filter(GoalSubscription.user_id == current_user.id, GoalSubscription.id == goal_sub_id).first()
    if not subscription:
        raise HTTPException(status_code=404, detail="Subscription not found")
    subscription.longitude = subscription.longitude
    subscription.latitude = subscription.latitude
    db.commit()
    db.refresh(subscription)
    return subscription

@router.delete("/subscription/{goal_sub_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_goal_subscription(
        goal_sub_id: int,
        current_user: User = Depends(get_current_user),
        db: Session = Depends(get_db)
    ):
